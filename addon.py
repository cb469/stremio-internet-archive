...
MIN_MOVIE_SIZE_MB = 150
MIN_SERIES_SIZE_MB = 50

# --- ADDON MANIFEST ---
MANIFEST = {
    "id": "org.yourname.internet-archive-streams",
    "version": "2.2.0", # Smart Scraper
...
def search_archive(query):
    # This function now searches a wider variety of collections to improve series results
...
def stream(type, id):
    ...
    if type == 'movie':
        ...
        # First, try a highly specific search for the episode
        specific_query = f'("{title} {s_e_simple}")'
        search_results = search_archive(specific_query)
        # If that fails, do a broad search for the series title
        if not search_results:
            search_results = search_archive(f'({title}) AND year:({year})')

    # Iterate through all found archive items to find the best files
    for result in search_results:
        ...
        for f in files:
            ...
            # For series, ensure the file is the correct episode
            if type == 'series':
                season_num, episode_num = id.split(':')[1:]
            ...
            valid_streams.append({
                ...
                "_size": int(f.get('size', 0)) # Internal key for sorting
            })
    
    # Sort by size (largest file first)
    sorted_streams = sorted(valid_streams, key=lambda k: k['_size'], reverse=True)
    # Remove the internal size key before sending to Stremio
